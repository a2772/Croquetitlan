...
import src.com.croquetitlan.bs.CatGastos;
import src.com.croquetitlan.bs.Trabajadores;
import src.com.croquetitlan.dao.DAOInitializationException;


@WebServlet("/promociones.view")
public class Promociones extends HttpServlet implements Servlet {
	...
	{
		List<src.com.croquetitlan.bs.Descuentos> promocionS=null;
		try {
			ConsultasBD consultas = new ConsultasBD();
			promocionS = consultas.obtenerDescuentos();
		...
		out.println("<h1 class=\"font-weight-bold mb-0\">Bienvenido a Promociones</h1>");
		out.println("<p class=\"lead text-muted\">Control de Promociones</p>");
		...
		out.println("<td colspan=\"9\">Croque-Rebajas mortales por tiempo limitado</th>");
		out.println("</tr>");
		//FIN TAITEL
		out.println("<tr class=\"tr-consultas-main\">");
		out.println("<th>Id Descuento</th>");
		out.println("<th>Producto</th>"); 
		out.println("<th>Fecha Inicio</th>");
		out.println("<th>Fecha Fin</th>"); 
		out.println("<th>Aplica para</th>");
		out.println("<th>Porcentaje</th>");
		out.println("<th>Precio Descuento</th>"); 
		out.println("<th>Unidades</th>");
		out.println("<th>Kilogramos</th>");
		out.println("</tr>");
		//Renglones
		for(int i=0;i<promocionS.size();i++) {
			if(i%2==0)
				out.println("<tr class=\"tr-consultas-one\">");
			else
				out.println("<tr class=\"tr-consultas-two\">");
			//Imprimimos todo en cada renglón
			out.println("<td>");out.println( promocionS.get(i).getIdDescuento() );out.println("</td>");
			out.println("<td>");out.println( promocionS.get(i).getNombre() );out.println("</td>");
			out.println("<td>");out.println( promocionS.get(i).getFechaInicio() );out.println("</td>");
			out.println("<td>");out.println( promocionS.get(i).getFechaFin() );out.println("</td>");
			out.println("<td>");out.println( promocionS.get(i).getSoloCliente() );out.println("</td>");
			out.println("<td>");out.println( promocionS.get(i).getPorcentaje() + "%" );out.println("</td>");
			out.println("<td>");out.println( "$" + promocionS.get(i).getPrecioDesc() );out.println("</td>");
			out.println("<td>");out.println( promocionS.get(i).getCantidadUnit() );out.println("</td>");
			out.println("<td>");out.println( promocionS.get(i).getCantidadGram() + " Kg" );out.println("</td>");
			//Cerramos renglón
			out.println("</tr>");
		} 
		out.println("</table>");            
		...
		out.println("<br><br>");
		out.println("<center>");
		out.println("<a class=\"boton-personalizado\" href=\"registroprom.view\">Agregar Promoción</a>");
		out.println("<a class=\"boton-personalizado\" href=\"\">Eliminar Promoción</a>");
