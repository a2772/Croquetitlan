...
import src.com.croquetitlan.bs.Producto;
import src.com.croquetitlan.bs.Trabajadores;


@WebServlet("/registrogasto.view")
public class RegistroGasto extends HttpServlet implements Servlet {
	...
	protected void doSomething(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try {
			...
			List<src.com.croquetitlan.bs.CatGastos> gS = consultas.obtenerCatGastos();
			//Inicio FORM
			out.println("<div class=\"container\">");
			out.println("<div class=\"content-center\">");
			out.println("<form action=\"inggasto.controller\" method=\"POST\">");
			//Desplegable productos
			out.println("<div class=\"form-group\">");
			out.println("<label for=\"producto\">Tipo de gasto</label>");
			out.println("<select required class=\"form-control\" name=\"idgasto\">");
			for(int i=0;i<gS.size();i++) {
				out.println("<option value=" + (i+1) + ">Gasto: " + gS.get(i).getGasto() + "</option>");
			}
			out.println("</select>");
			out.println("</div>");
			out.println("<div class=\"form-group\">");
			out.println("<label for=\"nombre\">Inserte el total a pagar</label>");
			out.println("<input required type=\"text\" class=\"form-control\" step=\"0.01\" min=0.00 name=\"pagar\" id=\"nombre\" placeholder=\"$ \">");
			out.println("</div>");
			out.println("<div class=\"form-group\">");
			out.println("<label for=\"nombre\">Seleccione la fecha limite el total a pagar</label>");
			out.println("<input required type=\"date\" class=\"form-control\" name=\"fechalimite\" id=\"nombre\" \">");
			out.println("</div>");
			out.println("<div class=\"form-group\">");
			out.println("<label for=\"nombre\">Estado del gasto</label>");
			out.println("<select required class=\"form-control\" name=\"pagado\">");
				out.println("<option value=1>No Pagado</option>");
				out.println("<option value=2>Pagado</option>");
			out.println("</select>");
			out.println("</div>");
			out.println("</div>");
			out.println("<br>");
