...
@WebServlet("/registroproductoexistencia.view")
public class RegistroProductoExistencia extends HttpServlet implements Servlet {
	...
	protected void doSomething(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String error = (String) request.getSession().getAttribute("errorinsproductoexistente");
		...
		out.println("</h1>");
		out.println("<p class=\"lead text-muted\">Formulario para Agregar Existencias</p>");
		out.println("</div>");
		...
		out.println("<form action=\"ingproductoexistente.controller\" method=\"POST\">");
		//	Desplegable productos
		out.println("<div class=\"form-group\">");
		out.println("<label for=\"producto\">Nombre del producto</label>");
		out.println("<select class=\"form-control\" name=\"producto\">");
		for(int i=0;i<catProductoS.size();i++)
			out.println("<option value=" + (i+1) + ">" + catProductoS.get(i).getNombre() + "</option>");
		out.println("</select>");
		out.println("</div>");
		//	FIN desplegable productos
		out.println("<div class=\"form-group\">");
		out.println("<label for=\"exampleFormControlInput6\">Fecha de Caducidad (Solo en alimentos)</label>");
		out.println("<input type=\"date\" class=\"form-control\" name=\"fechacaducidad\" id=\"exampleFormControlInput6\" placeholder=\"Ingresa la fecha en la que caducan los productos\">");
		out.println("</div>");
		out.println("<div class=\"form-group\">");
		out.println("<label for=\"exampleFormControlInput3\">Cantidad</label>");
		out.println("<input type=\"number\" class=\"form-control\" name=\"cantidad\" min=1  id=\"exampleFormControlInput3\" placeholder=\" Cantidad de productos\">");
		out.println("</div>");
		out.println("<div class=\"form-group\">");
		...
		out.println("</div>");
		if(error=="1")
			out.println("<label class=\"login-error\">  Valores no válidos  </label> ");
		out.println("</div>");
		out.println("<br>");
		out.println("<br>");
		out.println("<center>");
